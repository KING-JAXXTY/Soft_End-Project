...
function displayMyReports(reports) {
    ...
    container.innerHTML = reports.map(report => `
        <div class="card" style="cursor: pointer; padding: 0.75rem;" onclick="viewReportDetail('${report._id}')">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                <div style="flex: 1;">
                    <h3 style="font-size: 0.875rem; margin-bottom: 0.25rem; line-height: 1.3;">${report.subject.length > 35 ? report.subject.substring(0, 35) + '...' : report.subject}</h3>
                    <p style="font-size: 0.7rem; color: var(--text-secondary);">
                        ${new Date(report.createdAt).toLocaleDateString()}
                    </p>
                </div>
                <span class="badge" style="background: ${getStatusColor(report.status)}; font-size: 0.7rem; padding: 0.2rem 0.4rem;">
                    ${report.status}
                </span>
            </div>
            ...
            <div style="margin-top: 0.5rem;">
                ...
                <p style="font-size: 0.75rem; margin-top: 0.4rem; color: var(--text-secondary); 
            ...
            ${report.resolvedBy ? `
                <div style="margin-top: 0.5rem; padding: 0.4rem; background: var(--background); 
