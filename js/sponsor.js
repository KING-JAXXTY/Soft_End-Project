...
function displayMyReports(reports) {
    ...
    container.innerHTML = reports.map(report => `
        <div class="card" style="cursor: pointer; padding: 0.625rem;" onclick="viewReportDetail('${report._id}')">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.375rem;">
                <div style="flex: 1;">
                    <h3 style="font-size: 0.8125rem; margin: 0 0 0.25rem 0; line-height: 1.2; font-weight: 600;">${report.subject.length > 30 ? report.subject.substring(0, 30) + '...' : report.subject}</h3>
                    <p style="font-size: 0.6875rem; color: var(--text-secondary); margin: 0;">
                        ${new Date(report.createdAt).toLocaleDateString()}
                    </p>
                </div>
                <span class="badge" style="background: ${getStatusColor(report.status)}; font-size: 0.6875rem; padding: 0.1875rem 0.375rem; white-space: nowrap;">
                    ${report.status}
                </span>
            </div>
            
            <div style="margin-top: 0.375rem;">
                <p style="font-size: 0.6875rem; color: var(--text-secondary); margin: 0 0 0.25rem 0; font-weight: 600;">
                    ${report.reportType}
                </p>
                <p style="font-size: 0.6875rem; margin: 0; color: var(--text-secondary); 
            ...
            ${report.resolvedBy ? `
                <div style="margin-top: 0.375rem; padding: 0.375rem; background: var(--background); 
